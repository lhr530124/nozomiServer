...
def getRandomClans(score):
    rets = queryAll("SELECT `id`,icon,score,`type`,name,`desc`,members, `min`, creator FROM `nozomi_clan` WHERE `min`<=%s AND members>=4 AND members<10 LIMIT 50", (score,), 3)
    if rets==None or len(rets)==0:
        rets = queryAll("SELECT `id`,icon,score,`type`,name,`desc`,members, `min`, creator FROM `nozomi_clan` WHERE `min`<=%s AND members>=1 AND members<10 LIMIT 50", (score,), 3)
    return rets
...
def searchClans(text):
    ...
    if text=="":
        return None
    return queryAll("SELECT `id`,icon,score,`type`,name,`desc`,members,`min`,creator FROM `nozomi_clan` WHERE `name` LIKE %s AND members>0 LIMIT 50", (text+"%"), 3)
...
def getClanMembers(cid):
    rets = queryAll("SELECT `id`, score, lscore, name, memberType, level, totalCrystal, uglevel, dnum, rdnum FROM `nozomi_user` WHERE clan=%s", (cid), 3)
    return rets

def getClanInfo(cid):
    ret = queryOne("SELECT `id`, icon, score, `type`, name, `desc`, members, `min`, creator, state, statetime FROM `nozomi_clan` WHERE id=%s", (cid), 3)
    return ret

def joinClan(uid, cid):
    clan = list(getClanInfo(cid))
    user = queryOne("SELECT score FROM nozomi_user WHERE id=%s",(uid,), 3)
