...
def getUserState(uid):
    r = queryOne("SELECT shieldTime, onlineTime, attackTime FROM nozomi_user_state WHERE uid=%s", (uid,), 3)
    curTime = getTime()
    ret = dict()
    if r[0]>curTime:
...
def findAMatch(uid, score, scoreOff):
    ...
    while tryTime<3:
        ...
        if rscore>highScore:
            ...
            ids = queryAll("SELECT uid FROM nozomi_user_state WHERE uid!=%s AND shieldTime<%s AND attackTime<%s AND onlineTime<%s AND score>%s AND score<%s", (uid,curTime,curTime,curTime,minScore,maxScore), 3)
            if ids!=None:
                num = len(ids)
            ...
            if minScore<20:
                minScore = 5 
                maxScore = 75
            ids = queryOne("SELECT MIN(uid), MAX(uid) FROM nozomi_user_state WHERE score>%s AND score<%s", (minScore, maxScore), 3)
            if ids!=None:
                minId = ids[0]
                maxId = ids[1]
                if maxId != None and minId!=None:
                    cut = random.randint(minId, maxId)
                    ret = queryOne("SELECT uid FROM nozomi_user_state WHERE uid>=%s AND uid!=%s AND shieldTime<%s AND attackTime<%s AND onlineTime<%s AND score>%s AND score<%s LIMIT 1", (cut, uid, curTime, curTime, curTime, minScore, maxScore), 3)
                    if ret!=None:
